...
const SearchCustomer = () => {
  ...
  const handleSearch = async (e) => {
    ...
    if (!searchAadhar.trim()) {
      Swal.fire({
        icon: 'warning',
        title: 'Aadhar Required',
        text: 'Please enter an Aadhar number to search.',
    ...
    setLoading(true);
    try {
      const response = await axiosInstance.get(`/customer/get/${searchAadhar}`);
  ...
  const handleDelete = async (aadhar) => {
    ...
    if (result.isConfirmed) {
      try {
        await axiosInstance.delete(`/customer/delete/${aadhar}`);
        
        Swal.fire({
          icon: 'success',
          title: 'Customer Deleted!',
          text: 'Customer has been successfully removed from the system.',
          confirmButtonColor: '#2563eb',
        }).then(() => {
          setCustomerData(null);
          setSearchAadhar('');
    ...
    <div className="min-h-screen bg-gray-50 py-8">
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="mb-8">
          ...
          <p className="text-gray-600 mt-2">Find customer details using Aadhar number</p>
        </div>
        ...
        {customerData && (
          <div className="banking-card p-8">
            <div className="flex items-center justify-between mb-6">
              ...
              <div className="flex space-x-4">
                <Link
                  to={`/user/customer/edit/${customerData.aadharNumber}`}
                ...
                <button
                  onClick={() => handleDelete(customerData.aadharNumber)}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                ...
                <p className="text-lg text-gray-900">{customerData.name}</p>
              </div>
              
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Phone Number</label>
                <p className="text-lg text-gray-900">{customerData.phoneNumber}</p>
              </div>
              ...
              <div>
                ...
                <p className="text-lg text-gray-900">{customerData.aadharNumber}</p>
              </div>
              
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Date of Birth</label>
                <p className="text-lg text-gray-900">{new Date(customerData.dob).toLocaleDateString()}</p>
              </div>
            ...
            <div className="mt-6">
              ...
              <p className="text-lg text-gray-900">{customerData.address}</p>
            </div>
          </div>
        )}
      </div>
